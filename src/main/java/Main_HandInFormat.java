...
public class Main_HandInFormat {
    ...
    public static void main(String[] args) throws Exception {
        ...
        InputData inputData = InputData.readFile(inputFile);
        ...
        if(inputData.getTargetHeight() == 0) {
            String targetFile = args[nextInputIndex];
            nextInputIndex++;
            System.out.println("Reformat to target field " + targetFile);
            InputTarget inputTarget = InputTarget.readFile(targetFile);
...
//        printOfficialResult(schedule);
    ...
    private static void writeSolution(String file, List<FullMovement> schedule) throws IOException {
        String output =  "%CraneId;ContainerId;PickupTime;EndTime;PickupPosX;PickupPosY;EndPosX;EndPosY;\n";
        for(FullMovement f : schedule) {
            output += f.toString();
            output += "\n";
        }
